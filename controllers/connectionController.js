  ...
  try {
    ...
    if (!sender?.gender || !receiver.gender) {
      return res.status(400).json({
        success: false,
        message:
          "Both users must have their gender set to send a connection request",
      });
    }
    ...
    // Only allow male→female or female→male connections
    const validPair =
    ...
    if (!validPair) {
      return res.status(400).json({
        success: false,
        message: `Cannot send connection request. Users must be opposite genders (Male ↔ Female only).`,
      });
    }
...
const acceptRequest = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Connection accepted", connection });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const rejectRequest = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Connection rejected", connection });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const withdrawRequest = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Request withdrawn", connection });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
const getMyConnections = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, count: contacts.length, connections: contacts });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
